...
class SpotFlatTerrainPolicy(PolicyController):
    ...
    def __init__(
        ...
        self._decimation = 10  # Default decimation value
    ...
    def forward(self, dt, command):
        ...
        if self._policy_counter % self._decimation == 0:
            ...
            # Print hidden layer activations if using ModelWithHooks
            if hasattr(self.policy, 'get_activations'):
                activations = self.policy.get_activations()
                if activations:  # Should be 20Hz with decimation=10
                    elapsed_ms = (time.time() - self._start_time) * 1000
                    print(f"\n=== Neural Activity at {elapsed_ms:.1f}ms (counter={self._policy_counter}, decimation={self._decimation}) ===")
                    for layer_name, activation in activations.items():
                        print(f"{layer_name}: shape={activation.shape}, mean={activation.mean():.4f}, std={activation.std():.4f}")
                        # print(f"  Sample values: {activation[0][:5]}")  # Show first 5 values

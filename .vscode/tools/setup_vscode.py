...
ISAACLAB_DIR = '/home/chongxi/Isaacsim5/IsaacLab'
"""Path to the Isaac Lab directory."""

# Use current directory for output files
CURRENT_DIR = pathlib.Path(__file__).parents[2]
"""Path to the current directory (where .vscode folder is located)."""
...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        path_names = ['"' + ISAACLAB_DIR + '/' + rel_path + "/" + path_name + '"' for path_name in path_names]
    else:
        path_names = []
        print(
    ...
    path_names.extend(['"' + ISAACLAB_DIR + '/source/' + ext + '"' for ext in isaaclab_extensions])
...
def main():
    ...
    # write the Isaac Lab settings file to current directory
    isaaclab_vscode_filename = os.path.join(CURRENT_DIR, ".vscode", "settings.json")
    with open(isaaclab_vscode_filename, "w") as f:
        f.write(isaaclab_settings)

    # copy the launch.json file if it doesn't exist
    isaaclab_vscode_launch_filename = os.path.join(CURRENT_DIR, ".vscode", "launch.json")
